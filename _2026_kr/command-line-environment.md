...
당신은 `--all` 같은 긴 이름의 이중 대시 플래그와 대부분 단일 문자가 뒤에 오는 `-a` 같은 단일 대시 플래그를 볼 것입니다. 같은 옵션이 두 형식 모두로 지정될 수 있으며, `ls -a`와 `ls --all`은 동등합니다. 단일 대시 플래그는 종종 그룹화되므로, `ls -l -a`와 `ls -la`도 동등합니다. 플래그의 순서는 보통 중요하지 않으며, `ls -la`와 `ls -al`은 같은 결과를 생성합니다. 일부 플래그는 매우 흔하며, 셸 환경에 더 익숙해질수록, 당신은 본능적으로 그들에게 다가갈 것입니다. 예를 들어(`--help`, `--verbose`, `--version`).
...
가장 일반적인 글롭들은 와일드카드 `*`(0개 이상의 모든 것), `?`(정확히 1개의 모든 것), 그리고 곱슬머리 괄호입니다. 곱슬머리 괄호 `{}`는 쉼표로 분리된 패턴의 목록을 여러 인수로 확장합니다.
...
여기서 중요한 관찰은 세 프로그램이 모두 한 번에 실행된다는 것입니다. 즉, 셸이 먼저 cat을 호출하고, 그 다음 grep을 호출하고, 그 다음 uniq을 호출하지 않습니다. 대신, 세 프로그램이 모두 생성되고, 셸이 cat의 출력을 grep의 입력과 연결하고, grep의 출력을 uniq의 입력에 연결합니다. 파이프 연산자 `|`를 사용할 때, 셸은 체인에서 한 프로그램에서 다음 프로그램으로 흐르는 데이터 스트림에서 작동합니다.
...
특수한 신호는 `SIGKILL`입니다. 이것은 프로세스에 의해 캡처될 수 없으며 항상 즉시 종료시킬 것입니다. 하지만, 고아 자식 프로세스를 남기는 것 같은 나쁜 부작용을 가질 수 있습니다.

당신은 [여기](https://en.wikipedia.org/wiki/Signal_(IPC))에서 이들과 다른 신호들에 대해 더 배울 수 있습니다. 또는 [`man signal`](https://www.man7.org/linux/man-pages/man7/signal.7.html) 또는 `kill -l`을 입력하세요.

셸 스크립트 내에서, 당신은 신호들이 받아질 때 명령을 실행하기 위해 `trap` 내장을 사용할 수 있습니다. 이것은 정리 작업에 유용합니다:
...
프로그래머들이 그들의 일상 작업에서 원격 서버와 작업하는 것이 점점 더 일반적이 되었습니다. 이 작업을 위한 가장 일반적인 도구는 SSH(Secure Shell)입니다. 이것은 우리가 원격 서버에 연결하고 친숙한 셸 인터페이스를 제공합니다. 우리는 다음과 같은 명령으로 서버에 연결합니다:

```bash
ssh alice@server.mit.edu
```

여기서 우리는 서버 `server.mit.edu`에서 사용자 `alice`로 ssh하려고 합니다.

`ssh`의 종종 간과되는 기능은 명령을 비대화식으로 실행하는 능력입니다. `ssh`는 명령의 stdin을 보내고 stdout을 받는 것을 올바르게 처리하므로, 우리는 그것을 다른 명령들과 결합할 수 있습니다.
...
> 연결 해제, 수면 진입/종료, 네트워크 변경, 높은 지연 링크 처리를 관리할 수 있는 SSH 대체로 [`Mosh`](https://mosh.org/) 설치를 시도해 보세요.

`ssh`가 우리가 원격 서버에서 명령을 실행하도록 하려면, 우리는 우리가 그렇게 할 권한이 있음을 증명해야 합니다. 우리는 비밀번호 또는 ssh 키를 통해 이를 할 수 있습니다. 키 기반 인증은 공개 키 암호화를 활용하여 클라이언트가 비밀 개인 키를 소유하고 있음을 서버에 증명합니다. 키 기반 인증은 더 편리하고 더 안전하므로, 당신은 그것을 선호해야 합니다. 개인 키(종종 `~/.ssh/id_rsa`이고 더 최근에는 `~/.ssh/id_ed25519`)가 효과적으로 당신의 비밀번호이므로, 그렇게 취급하고 절대 그 내용을 공유하지 마세요.

한 쌍을 생성하려면, 당신은 [`ssh-keygen`](https://www.man7.org/linux/man-pages/man1/ssh-keygen.1.html)을 실행할 수 있습니다.
```bash
ssh-keygen -a 100 -t ed25519 -f ~/.ssh/id_ed25519
```

당신이 GitHub에 SSH 키를 사용하여 푸시하도록 구성한 적이 있다면, 당신은 아마도 [여기](https://help.github.com/articles/connecting-to-github-with-ssh/)에 설명된 단계들을 했을 것이고, 이미 유효한 키 쌍을 가지고 있을 것입니다. 당신이 암호 문구가 있는지 확인하고 유효성을 검사하려면, 당신은 `ssh-keygen -y -f /path/to/key`를 실행할 수 있습니다.

서버 측에서, `ssh`는 어떤 클라이언트를 허용할지 결정하기 위해 `.ssh/authorized_keys`를 봅니다. 공개 키를 복사하려면, 당신은 다음을 사용할 수 있습니다:
...
명령을 실행하는 것 이상으로, ssh가 확립하는 연결은 서버로부터 그리고 파일들을 안전하게 전송하는 데 사용될 수 있습니다. [`scp`](https://www.man7.org/linux/man-pages/man1/scp.1.html)는 가장 전통적인 도구이고, 문법은 `scp path/to/local_file remote_host:path/to/remote_file`입니다. [`rsync`](https://www.man7.org/linux/man-pages/man1/rsync.1.html)는 로컬과 원격에서 동일한 파일들을 감지하고, 그들을 다시 복사하는 것을 방지함으로써 `scp`를 개선합니다. 이것은 또한 심볼릭 링크, 권한에 대해 더 미세하게 조정된 제어를 제공하며, `--partial` 플래그 같은 추가 기능들을 가지고 있습니다. 이것이 이전에 중단된 복사를 재개할 수 있습니다. `rsync`는 `scp`와 유사한 문법을 가집니다.

SSH 클라이언트 구성은 `~/.ssh/config`에 위치하며, 이것이 호스트들을 선언하고 그들을 위해 기본 설정들을 설정할 수 있게 합니다. 이 구성 파일은 `ssh`뿐만 아니라 다른 프로그램들 같은 `scp`, `rsync`, `mosh`에 의해서도 읽혀집니다.
...
명령줄 인터페이스를 사용할 때, 당신은 종종 한 번에 하나 이상의 것을 실행하고 싶을 것입니다. 예를 들어, 당신은 에디터와 당신의 프로그램을 나란히 실행하고 싶을 수 있습니다. 비록 이것이 새로운 터미널 창을 열어서 달성될 수 있지만, 터미널 멀티플렉서를 사용하는 것이 더 다양한 솔루션입니다.

[`tmux`](https://www.man7.org/linux/man-pages/man1/tmux.1.html) 같은 터미널 멀티플렉서는 당신이 효율적인 방식으로 여러 셸 세션과 상호 작용할 수 있도록 창과 창으로 터미널 창을 멀티플렉스할 수 있게 합니다. 더욱이, 터미널 멀티플렉서는 당신이 현재 터미널 세션을 분리하고 나중에 시간의 어느 지점에 다시 연결할 수 있게 합니다. 이러한 이유로, 터미널 멀티플렉서는 원격 머신들과 작업할 때 정말로 편리합니다. 이것은 `nohup`과 유사한 트릭을 사용할 필요를 피합니다.

가장 인기 있는 터미널 멀티플렉서는 이러한 날에 [`tmux`](https://www.man7.org/linux/man-pages/man1/tmux.1.html)입니다. `tmux`는 매우 커스터마이징 가능하고, 연관된 키바인딩을 사용하여, 당신은 여러 탭과 창을 생성하고 빠르게 그들을 네비게이트할 수 있습니다.

`tmux`는 당신이 그 키바인딩들을 알기를 기대합니다. 그리고 그들 모두는 `<C-b> x` 형태입니다. 여기서 이것은 (1) `Ctrl+b`를 누르고, (2) `Ctrl+b`를 놓고, 그 다음 (3) `x`를 누르는 것을 의미합니다. `tmux`는 객체의 다음 계층 구조를 가집니다:
- **세션** - 세션은 하나 이상의 창이 있는 독립적인 작업 공간입니다
    ...
    + `tmux a`는 마지막 세션을 첨부합니다. 당신은 어떤 것을 지정하기 위해 `-t` 플래그를 사용할 수 있습니다.

- **창** - 편집자나 브라우저의 탭과 동등하며, 같은 세션의 시각적으로 분리된 부분입니다.
    + `<C-b> c`는 새로운 창을 생성합니다. 닫으려면, 당신은 단지 `<C-d>`로 셸들을 종료할 수 있습니다.
    + `<C-b> N`은 _N_번째 창으로 이동합니다. 주의: 그들은 번호가 매겨집니다.
    + `<C-b> p`는 이전 창으로 이동합니다.
    + `<C-b> n`은 다음 창으로 이동합니다.
    + `<C-b> ,`은 현재 창의 이름을 바꿉니다.
    + `<C-b> w`는 현재 창들을 나열합니다.

- **창** - vim 분할 같은 창들은 당신이 같은 시각적 디스플레이에서 여러 셸들을 가질 수 있게 합니다.
    ...
    + `<C-b> [`는 스크롤백을 시작합니다. 당신은 그 다음 `<space>`를 눌러 선택을 시작할 수 있고 `<enter>`로 그 선택을 복사할 수 있습니다.
...
tmux와 SSH를 당신의 도구 키트에 가지고, 당신은 어떤 머신에서든 당신의 환경이 집처럼 느껴지도록 하고 싶을 것입니다. 그것이 셸 커스터마이제이션이 나오는 곳입니다.
...
1. 프로세스 치환 `<(command)`는 명령의 출력을 파일인 것처럼 사용할 수 있게 합니다. `diff`를 프로세스 치환과 함께 사용하여 `printenv`와 `export`의 출력을 비교하세요. 왜 그들이 다른가요? (힌트: `diff <(printenv | sort) <(export | sort)`를 시도해 보세요).
